...
class BooksApp extends React.Component {
  ...
  BookList() {
    const { addBook, Bookshelf } = this,
      ...
      bookshelves = SectionEnum.asList
        .filter(section => section.id !== SectionEnum.NONE)
        .map(section => Bookshelf({
          id: section.id,
          title: section.name,
          section: section.id,
          books: books.filter((book) => book.section === section.id).sort(sortBookByTitle)
        }));
  ...
  Bookshelf({id, title, section, books}) {
    const { Book } = this,
  ...
  Book({id, url, title, author, section}) {
