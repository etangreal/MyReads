...
class BooksApp extends React.Component {
  ...
  state = {
    ...
    books: [{
        ...
        author: 'Harper Lee',
        shelf: ShelfEnum.CURRENTLY_READING
      }, {
        id: 2,
    ...
    const { addBook, Bookshelf } = this,
      ...
      bookshelves = ShelfEnum.asList
        ...
        .map(shelfEnum => Bookshelf({
          ...
          shelf: shelfEnum.id,
          books: books.filter((book) => book.shelf === shelfEnum.id).sort(sortBookByTitle)
        }));
  ...
  Bookshelf({id, title, shelf, books}) {
    const { Book } = this,
  ...
  onSelectionChange(id, value) {
    ...
    books[index].shelf = Number(value);
    this.setState({books});
  }
